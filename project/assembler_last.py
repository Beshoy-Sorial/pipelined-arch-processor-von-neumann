class Assembler:
    ...
    def assemble_line(self, line):
        ...
        if line.startswith(".ORG"):
            self.pc = self.parse_number(line.split()[1])
            return

        # ---------- DATA WORD (number only) ----------
        try:
            self.parse_number(line)
            self.write(self.to_32bit(line))
            return
        except ValueError:
            pass
        ...
        instr = parts[0].upper()
        # ---------- INT0 / INT1 ----------
        if instr.startswith("INT") and len(instr) > 3:
            index = instr[3:]  # "0" or "1"
            opcode = self.opcodes["INT"]
